...
import { fetchListCoinData, useSelectListCoin } from "../Redux/Slices/ListCoin";
...
const CoinList = () => {
  ...
  const { id } = useParams();
  const { isLoading, listCoinData, isError } = useSelector(useSelectListCoin);
  ...
  const npage = Math.ceil(listCoinData.length / coinPerPage);
  ...
  useEffect(() => {
    dispatch(fetchListCoinData());
  }, [dispatch, id, selectedCurrency, SelectedTime]);
  ...
  return (
    <>
      {Object.entries(listCoinData).length > 0 && (
        <section>
          <div className="mb-3 mt-3 flex justify-between items-center">
            <span className="text-2xl font-bold">Market</span>
            <div className=" flex gap-2">
